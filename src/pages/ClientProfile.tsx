...
import { Building2, FolderOpen, Globe, TrendingUp, Key, Calendar, FileText, Star, Phone, Mail, MapPin, ExternalLink, Edit, Plus, Users, Handshake } from 'lucide-react';
...
const ClientProfile: React.FC = () => {
  ...
  const handleAddAccess = async (data: {
    plataforma?: string;
    usuario?: string;
    senha?: string;
    notas?: string;
  ...
  const transformedClient = client ? {
    ...
    cover: '',
    status: client.status === 'ativo' ? 'active' as const : client.status === 'inativo' ? 'inactive' as const : 'onboarding' as const,
    size: client.porte === 'micro' ? 'MEI' as const : client.porte === 'pequeno' ? 'PME' as const : 'large' as const,
    ...
    contractType: client.tipo_contrato === 'recorrente' ? 'recurring' as const : client.tipo_contrato === 'projeto_unico' ? 'project' as const : 'one-time' as const,
    temperature: client.temperatura === 'quente' ? 'hot' as const : client.temperatura === 'morno' ? 'warm' as const : 'cold' as const,
  ...
  return <div className="container mx-auto p-6 space-y-6">
        ...
        <CardHeader>
          <div className="flex items-start justify-between">
            <div className="flex items-center space-x-4">
              {client.logo_url ? <img src={client.logo_url} alt={client.nome} className="w-16 h-16 rounded-lg object-cover" /> : <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center">
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="pages">
          <Card>
            ...
            <CardContent>
              {pageLinks.length === 0 ? <div className="text-center py-8 text-muted-foreground">
                  ...
                  <TableBody>
                    {pageLinks.map(pageLink => <TableRow key={pageLink.id}>
                        <TableCell className="font-medium">{pageLink.title}</TableCell>
                        <TableCell>
                          <Badge variant="outline">
                            {pageLink.type === 'website' ? 'Website' : pageLink.type === 'campaign' ? 'Campanha' : 'Landing Page'}
                          </Badge>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <div className="flex space-x-2">
                            <Button variant="outline" size="sm" onClick={() => window.open(pageLink.link, '_blank')}>
        ...
        <TabsContent value="access">
          <Card>
            ...
            <CardContent>
              {accesses.length === 0 ? <div className="text-center py-8 text-muted-foreground">
                  <Key className="w-12 h-12 mx-auto mb-4 opacity-50" />
                  <p>Nenhum acesso cadastrado ainda.</p>
                </div> : <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {accesses.map(access => <Card key={access.id} className="hover:shadow-md transition-shadow">
                      <CardContent className="pt-4">
                        <div className="space-y-2">
                          ...
                          <div className="space-y-1">
                            ...
                            {access.notas && <div className="flex items-center space-x-2">
      ...
      {/* Dialogs */}
      <PageLinkDialog open={pageLinkDialogOpen} onOpenChange={setPageLinkDialogOpen} pageLink={selectedPageLink} onSave={selectedPageLink ? data => updatePageLink(selectedPageLink.id, data) : addPageLink} />

      <AccessDialog open={accessDialogOpen} onOpenChange={setAccessDialogOpen} access={selectedAccess} onSave={selectedAccess ? handleUpdateAccess : handleAddAccess} />

      {transformedClient && <ClientEditDialog open={editDialogOpen} onOpenChange={setEditDialogOpen} client={transformedClient} onSave={handleEditClient} />}
    </div>;
};
